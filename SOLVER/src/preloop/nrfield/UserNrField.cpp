...
int UserNrField::getNrAtPoint(const RDCol2 &coords) const {
    ...
    double rearth = mParameters[0];
    double rhigh = rearth - mParameters[1];
    double rlow = rearth - mParameters[2];
    int nuhigh = mParameters[3];
    int nulow = mParameters[4];
    ...
    if (r > rhigh) {
        ...
        nu = round((nuhigh - nulow) / (rhigh - rlow) * (r - rlow) + nulow);
    }
...
std::string UserNrField::verbose() const {
    ...
    ss << "  Type                     =   User-defined 5 parameters" << std::endl;
